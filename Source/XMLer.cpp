...
void XMLer::drawGraph(){
    if (!resultTextEdit->toPlainText().isEmpty()) {
        ...
        // Create a new dialog to show the social network graph
        // QDialog* socialNetworkDialog = new QDialog(this);
        // socialNetworkDialog->setWindowTitle("Social Network Graph");

        // // // Create an instance of SocialNetworkGraph with parsed vertices
        // // SocialNetworkGraph* socialNetworkGraph = new SocialNetworkGraph(parsed, socialNetworkDialog);
        ...
        for(int i = 0; i < (xmlLines[j]).size(); i++)
        {
            ...
            if (xmlLines[j][i] == '<' && xmlLines[j][i + 1] != '/') // open tag
            {
                ...
                if(tag=="body")
                {
                    ...
                    {
                        // userVector[USERNUM].posts[postNumber].append(xmlLines[j]);
                        content.append(xmlLines[j]);
                        j++;
                    }
                    userVector[userNum].posts.push_back(content);
                    // qDebug() << userVector[USERNUM].posts[postNumber];
                }
                if(tag=="topic")
                {
                    //topicNumber++;
                    ...
                    //qDebug() << userVector[USERNUM].topics[postNumber][topicNumber];
                }
            }
        }
    }
...
void XMLer::createGraphVisualization(const QVector<Vertex> &vec)
{
    ...
    dotFile_Str += temp + "record ," + temp + "  color=" + temp + "black" \
                   +  temp +  "] \n\n";

    for(const auto& v : vec)
    {
        dotFile_Str += v.user_id + "[ label = " + temp + "\"{Name: " + v.user_name + " | ID: " \
                       + v.user_id + "} \"" + temp  + "]\n" + v.user_id + " -> {";
